...
public class UserController {
	...
	public String signUp(ModelMap model, @ModelAttribute("user") User user) {	
		...
		try {			
			...
			if(check){ 			
				user.setMatKhau(user.getMatKhau().toLowerCase()); // chuyển mật khẩu thành chữ thường****hash mk
...
//		System.out.println(gRecaptchaResponse);
		...
		if(check){ 							
			...
			query.setParameter("m", user.getMatKhau());//**hash mk
			query1.setParameter("aemail",user.getEmail() );
			query1.setParameter("pemail",user.getMatKhau() );//**hash mk
